...
{
    class SuperficialGeoDrawingTemplate : DrawingTemplate
    {
        public SuperficialGeoDrawingTemplate(TileProvider provider) : base(provider)
        { }

        public override SKSurface DrawImage(int MMWidth, int MMHeight, WGS84 location)
        {
            using (SKSurface map = _tileProvider.GetScaleImage(5000, MMWidth, MMHeight, new string[] { "OSM", "superficialgeo" }, location).Result)
            {
        ...
        {
            return "Superficial Geology 1:5000";            
        }
    }
}
